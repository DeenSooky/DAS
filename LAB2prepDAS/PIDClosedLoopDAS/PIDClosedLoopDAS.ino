...
#include <SPI.h> 
#include <PID_v1.h>    /* Needed to communicate with LS7366R (Counter Click) */
...
/* Intervals in ms for user-defined timed loops*/
const int printInterval_CL=20;       
...
double measuredPosFromEncoder2=0;
double positionSetPoint;
double percentDutyCycle;
double error=positionSetPoint-measuredPosFromEncoder2;

//PID
double controlInterval;
double Input=measuredPosFromEncoder2;
double Output=percentDutyCycle;
...
void loop() 
{
  ...
  if ( currentMillis - prevMillisPrint >= printInterval_CL )
  {
...
void controlLoop()
{
   ...
   measuredPosFromEncoder2 = readEncoderCountFromLS7366R();
   myPID.Compute();
   Serial.print("Count from LS7366R = ");
   Serial.print(measuredPosFromEncoder2);
   Serial.print("\r\n");
}
